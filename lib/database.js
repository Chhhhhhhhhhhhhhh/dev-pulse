...
function init() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS tasks (
      ...
      source TEXT DEFAULT 'local',  -- 'local', 'github'
      source_id TEXT DEFAULT '',  -- GitHub issue/PR number
  ...
  if (zombies.length > 0) {
    ...
    const stmt = db.prepare(
      "UPDATE focus_sessions SET ended_at = ?, completed = 0, notes = 'Auto-closed: server restart' WHERE ended_at IS NULL"
    );
    stmt.run(now);
    console.log(`[DB] Auto-closed ${zombies.length} zombie focus session(s)`);
  }
